...
public class JanelaCliente extends JFrame {
  ...
  protected JButton btnPonto = new JButton("Ponto"), btnLinha = new JButton("Linha"),
      ...
      btnAbrirNuvem = new JButton("Abrir nuvem"), btnSalvarNuvem = new JButton("Salvar nuvem");
  ...
  */
  public JanelaCliente(Parceiro servidor) {
    ...
    try {
      Image btnAbrirNuvemImg = ImageIO.read(getClass().getResource("resources/abrir.jpg"));
      btnAbrirNuvem.setIcon(new ImageIcon(btnAbrirNuvemImg));
    } catch (IOException e) {
      JOptionPane.showMessageDialog(null, "Arquivo abrir.jpg não foi encontrado", "Arquivo de imagem ausente",
          JOptionPane.WARNING_MESSAGE);
    }
    ...
    try {
      Image btnSalvarNuvemImg = ImageIO.read(getClass().getResource("resources/salvar.jpg"));
      btnSalvarNuvem.setIcon(new ImageIcon(btnSalvarNuvemImg));
    } catch (IOException e) {
      JOptionPane.showMessageDialog(null, "Arquivo salvar.jpg não foi encontrado", "Arquivo de imagem ausente",
          JOptionPane.WARNING_MESSAGE);
    }
    ...
    btnAbrirNuvem.addActionListener(new AbrirNuvem(servidor));
    btnSalvarNuvem.addActionListener(new SalvarNuvem(servidor));
    ...
    pnlBotoes.add(btnAbrirNuvem);
    pnlBotoes.add(btnSalvarNuvem);
...
/**
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (result == 0) {
        ...
        if (selectedFile.exists()) {
          int resposta = JOptionPane.showConfirmDialog(null, "Deseja substituir o arquivo existente?", "title",
          ...
          if (resposta != 0) {
            ...
            salvar.actionPerformed(e); // E DEPOIS EU DOU O START NO PERFORMED PQP! ISSO É ALGO MT UTIL PRA MINHA VIDA
                                       // TIO!de qualquer um em java takeoparauba kkkk
      ...
      if (result == 1) {
      } // não faz nada, por que o usuário desistiu (se eu não colocar, ele abre
        // novamente o Jfile)
  ...
  */
  protected class AbrirNuvem implements ActionListener {
    ...
    public AbrirNuvem(Parceiro servidor) {
      this.servidor = servidor;
    }

    public void actionPerformed(ActionEvent e) {

      Desenhos desenhos = null;
      ...
      try {
        ...
        desenhos = (Desenhos) servidor.envie();
      ...
      frame.setTitle("Desenhos salvos na nuvem");
    ...
    public void desenharNaTela(String string) {
      ...
      while (itens.hasMoreTokens()) {
        ...
        if ((item.contains("ponto"))) {
          ...
          Color cor = new Color(Integer.parseInt(array1[3]), Integer.parseInt(array1[4]),
              Integer.parseInt(fimProg.replace("].fga", "")));
  ...
  */
  protected class SalvarNuvem implements ActionListener {
    ...
    public SalvarNuvem(Parceiro servidor) {
      this.servidor = servidor;
    }
    public void actionPerformed(ActionEvent e) {
      figuras.clear();
